...
Topic: Constructive Algorithms / Implementation (1300)

Approach: Checkerboard / Chess board generation pattern while k > 0.

If we have placed all k islands, yes and print solution, else no.
...
int main () {
    ...
    // read in constraints and initialize the board
    int n, k; cin >> n >> k;
    vector<vector<char>> grid(n,vector<char>(n,'S'));
    int x,y;
    
    // perform pattern generation (maximizing the islands possible)
    range(0,n,2,x) {
        range(0,n,2,y) {
            // if already placed, skip. no need to go over diagonal, it is already processed.
    ...
    // if we couldn't make k islands, no.
    if (k > 0) {
        cout << "NO\n";
    
    // if we could, yes, and print the grid.
    } else {
        cout << "YES\n";
        range(0,n,1,x) {
            range(0,n,1,y) {
