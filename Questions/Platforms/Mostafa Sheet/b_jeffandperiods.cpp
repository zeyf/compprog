...
int main () {
    ...
    range(1, n+1, 1, x) {
        int u;  cin >> u;
        int size = sz(values[u]);
        if (size <= 1) {
            values[u].add(x);
        } else if (!bad.co(u)) {
            if (values[u][size-1]-values[u][size-2] != x - values[u][size-1])
                bad.ins(u);
            else
                values[u].add(x);
        };
    ...
    for (int x = 0; x < 100001; ++x) {
        if (sz(values[x])==1) { ++valid; }
        else if (sz(values[x])>1 && !bad.count(x)) { ++valid; };
    };

    cout << valid nl;

    for (int x = 0; x < 100001; ++x) {
        if (sz(values[x])==1) cout << x << " " << 0 << "\n";
        else if (sz(values[x])>1 && !bad.count(x)) cout << x << " " << values[x][1]-values[x][0] << "\n";
    };
